...
const ScientificCalculator: React.FC = () => {
    ...
    try {
      ...
      switch (op) {
        ...
        case "ln":
          if (currentNum <= 0) {
            throw new Error(
              "Cannot calculate natural logarithm of a non-positive number"
            );
          }
  ...
  const handleEquals = () => {
    try {
      ...
      // Ensure the expression ends with the current display number
      const fullExpression = expression.trim() + display;
      // Use mathjs.evaluate for safe calculation
      // Replace visual operator '×' with '*' if necessary, handle power '^'
      const sanitizedExpression = fullExpression
        .replace(/×/g, "*") // Replace visual multiplication if used
        .replace(/\^/g, "^"); // Keep power operator for mathjs

      const result = evaluate(sanitizedExpression);

      // mathjs handles errors, but check for NaN/Infinity just in case
  ...
  return (
    <ToolLayout toolName="scientificCalculator">
      {/* MetaTags removed, handled by ToolLayout using dataToolsDescription.ts */}
